---
layout: home

hero:
  name: Manboster
  text: Your Personal Manbo Lobster!
  tagline: 
  actions:
    - theme: brand
      text: Get Started
      link: /docs/quickstart.md
    - theme: alt
      text: Documentation
      link: /docs/index.md
    - theme: alt
      text: View on GitHub
      link: https://github.com/manboster/manboster
features:
  - icon: ⚡️
    title: Fast
    details: Thanks to Golang, Manboster is born with genes that is fast and multithreaded. Also, it consumes only 40MB memory in your machine when idle. 
  - icon: 🔐
    title: Secure
    details: Manboster features built in Zero Trust design. If you disallow the LLM to do anything, it won't do anything due to the limitation of Gatekeeper.
  - icon: ⚖️
    title: Guard Inside
    details: Manboster features Hachimi, a guard model mainly running locally in your machine.  You can handle all requests to Hachimi, it will help you handle LLM's tool call requests properly unless it's unsure or unsafe.
  - icon: 🔧
    title: Easy to use
    details: Out of the box, only one executable file. Just download and click to start. Also, we are working on websocket-based web panel, stay tuned!
---
